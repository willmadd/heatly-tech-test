...
const Canvas = ({ dataCategory }: Props) => {
  ...
  useEffect(() => {
    ...
    // Generate cylinder vertices for all markers
    let allCylinderVertices: number[] = [];
    let allCylinderColors: number[] = [];

    const processedMarkers: MarkerData[] = markers.map((marker) => ({
      ...marker,
      x: ((marker.lon + 180) / 360) * 1.5 - 0.75, // Convert longitude to x
      y: (-(marker.lat + 90) / 180) * 1.5 + 0.75, // Convert latitude to y
      h: 0, // This will be set by the data category
