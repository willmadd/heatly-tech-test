...
const normaliseLatLng = (lat: number, lng: number) => {
  // Convert longitude from [-180, 180] to [-0.75, 0.75] to match the quad dimensions
  const x = (lng / 180) * 0.75;

  // Convert latitude from [-90, 90] to [-0.75, 0.75]
  // We use lat directly since positive latitude should be in upper hemisphere
  const y = (lat / 90) * 0.75;

  return { x, y };
};

const normaliseValues = (value: number, min: number, max: number) => {
...
const markers = data.map((val) => {
  ...
  return {
    ...
    [DataCategoryEnum.Gdp]: normaliseValues(val.gdp, minGdp, maxGdp), // removed .toFixed(2)
    [DataCategoryEnum.Area]: normaliseValues(val.area, minArea, maxArea), // removed .toFixed(2)
